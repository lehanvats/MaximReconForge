      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={() => {
              if (loading || error || !markdown) return;
              const doc = new jsPDF();
              doc.text(markdown, 10, 10);
              doc.save(`maximrecon_${(target || "target").replace(/\./g, "_")}_report.pdf`);
            }}
            className="inline-flex shrink-0 items-center gap-2 rounded-full px-5 py-3 text-[12px] font-bold bg-white text-[#181468] hover:bg-white/90"
          >
            ↓ Download .pdf
          </button>
        </div>
