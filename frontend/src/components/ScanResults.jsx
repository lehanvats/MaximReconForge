...
function generateFormattedPdf(markdownText, targetDomain) {
  ...
  doc.setFontSize(16);
  doc.setTextColor(24, 20, 104);
  doc.text(`MaximReconForge Report: ${targetDomain || "Target"}`, margin, 20);

  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(120, 120, 120);
  doc.text(`Generated on ${new Date().toLocaleDateString()} | Threat Intelligence Assessment`, margin, 26);
  ...
  for (let line of rawLines) {
    ...
    for (let wrapped of wrappedLines) {
  ...
  const filename = `maximrecon_${(targetDomain || "target").replace(/\./g, "_")}_report.pdf`;
  doc.save(filename);
}

function downloadMarkdownFile(markdownText, targetDomain) {
  const blob = new Blob([markdownText || ""], { type: "text/markdown;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const link = document.createElement("a");
  link.href = url;
  link.download = `maximrecon_${(targetDomain || "target").replace(/\./g, "_")}_report.md`;
...
function ReportPanel({ loading, error, markdown, target }) {
  const disabled = loading || error || !markdown;
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <ResultHeading
          ...
          description="The final security assessment generated by the reporting agent. Read it below or download in your preferred format."
        />
        ...
        <div className="flex gap-2">
          <button
            type="button"
            disabled={disabled}
            onClick={() => downloadMarkdownFile(markdown, target)}
            className={`
          ...
          <button
            type="button"
            disabled={disabled}
            onClick={() => generateFormattedPdf(markdown, target)}
            className={`
              inline-flex shrink-0 items-center gap-2
              rounded-full px-5 py-3 text-[12px] font-bold
              transition
